...
`;
    ...
    if (!response.text) {
      throw new ApiError(500, 'No response text from Gemini');
    }
    ...
    if (processBreakdown.stages.length !== 5) {
      throw new ApiError(400, 'Expected 5 stages from Gemini');
    }
    ...
    throw new ApiError(500, `Failed to generate process: ${error.message}`);
  }
}
...
`;
    ...
    if (!quizData.questions || quizData.questions.length < 5 || quizData.questions.length > 7) {
      throw new ApiError(400, 'Invalid quiz data: expected 5-7 questions');
    }
    
    // Validate each question
    quizData.questions.forEach((q, index) => {
      if (!q.question || !q.options || q.options.length !== 4) {
        throw new ApiError(400, `Invalid question ${index}: missing required fields`);
      }
      if (q.correctAnswer < 0 || q.correctAnswer > 3) {
        throw new ApiError(400, `Invalid question ${index}: correctAnswer must be 0-3`);
      }
      if (q.svgReference !== null && (q.svgReference < 0 || q.svgReference >= processData.stages.length)) {
        throw new ApiError(400, `Invalid question ${index}: svgReference out of bounds`);
      }
    });
